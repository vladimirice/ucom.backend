...
class OrganizationService {
    ...
    async processNewOrganizationCreation(req) {
        ...
        const { newOrganization, newUserActivity, boardInvitationActivity } = await db
            ...
            const usersTeam = await usersTeamService.processNewModelWithTeam(newModel.id, OrganizationService.getEntityName(), body, this.currentUser.id, transaction);
            const usersTeamIds = usersTeamService.getUsersTeamIds(usersTeam);
            const newUserActivity = await usersActivity.processNewOrganization(req.signed_transaction, this.currentUser.id, newModel.id, transaction);
            const boardInvitationActivity = [];
            for (let i = 0; i < usersTeamIds.length; i += 1) {
                const res = await usersActivity.processUsersBoardInvitation(this.currentUser.id, usersTeamIds[i], newModel.id, transaction);
     ...
     * @private
     */
    async sendOrgCreationActivityToRabbit(newUserActivity) {
     ...
     * @private
     */
    async sendOrgTeamInvitationsToRabbit(boardInvitationActivity) {
    ...
    async updateOrganization(req) {
        ...
        const { updatedModel, boardInvitationActivity } = await db
            ...
            const deltaData = await usersTeamService.processUsersTeamUpdating(orgId, organizationsModelProvider.getEntityName(), body, this.currentUser.id, transaction);
            if (updatedCount !== 1) {
     ...
     * @private
     */
    async processUsersTeamInvitations(usersToAddFromRequest, orgId, transaction) {
        ...
        const boardInvitationActivity = [];
        for (let i = 0; i < usersTeamIds.length; i += 1) {
            const res = await usersActivity.processUsersBoardInvitation(this.currentUser.id, usersTeamIds[i], orgId, transaction);
            boardInvitationActivity.push(res);
        }
     ...
     * @return {Object}
     */
    getRequestBodyWithFilenames(req) {
        const body = req.body;
        // Lets change file
    ...
    addSourceAvatarFilenameToBody(file, body) {
     ...
     * @private
     */
    async checkUniqueFields(values, organizationId = null) {
     ...
     * @private
     */
    makeEmptyStringUniqueFieldsNull(body) {
        const uniqueFields = organizationsRepositories.Main.getOrganizationModel().getUniqueFields();
        uniqueFields.forEach((field) => {
            if (body[field] === '') {
                body[field] = null;
     ...
     * @param {number} userId
     */
    async checkUpdatePermissions(orgId, userId) {
