...
class OrganizationService {
  ...
  async updateOrganization(req) {
    ...
    return await db
      .transaction(async transaction => {
        ...
        await UsersTeamService.processUsersTeamUpdating(org_id, OrganizationsModelProvider.getEntityName(), body, this.currentUser.id, transaction);
  ...
  async getAllForPreview(query) {
    ...
    const models = await OrganizationsRepositories.Main.findAllForPreview(params);
    OrgPostProcessor.processManyOrganizations(models);

    const modelsTotalAmount = await OrganizationsRepositories.Main.countAllOrganizations();

    let metadata = {
      total_amount: modelsTotalAmount,
      page:         +query['page'],
      per_page:     +query['per_page'],
      has_more:     params.offset + params.limit < modelsTotalAmount,
    };
  ...
  async findOneOrgByIdAndProcess(modelId) {
    ...
    this._processOneAfterQuery(model, this.currentUser.id, activityData);
   ...
   * @private
   */
  _processOneAfterQuery(model, currentUserId, activityData) {
    if (!model) {
      return null;
    }
    ...
    UserPostProcessor.processModelAuthor(model, currentUserId);
