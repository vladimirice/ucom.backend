...
class OrganizationService {
  ...
  async findOneByIdAndProcess(postId) {
    ...
    this._processOneAfterQuery(model);
 ...
 async  _processUserRequest(req) {
    ...
    await this._checkUniqueFields(value);
  ...
  async _checkUniqueFields(values) {
    ...
    const existed = await OrganizationsRepositories.Main.findOneWithUniqueFields(toFind);

    if (existed) {
      let errors = [];

      uniqueFields.forEach(field => {
        if (existed[field] === toFind[field]) {
          errors.push({
            field,
