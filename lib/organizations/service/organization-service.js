...
const NotificationsCreator          = require('../../entities/service').NotificationsCreator;
...
class OrganizationService {
  ...
  async processNewOrganizationCreation(req) {
    ...
    const { newOrganization, newUserActivity, usersTeamIds } = await db
      .transaction(async transaction => {
        ...
        await NotificationsCreator.processOrgBoardInvitationForUsers(usersTeamIds, newModel.id, transaction);
    ...
    try {
      await NotificationsCreator.sendUnreadMessagesNotificationsViaSockets(usersTeamIds);
    } catch (err) {
      console.warn('Error related to sockets. Not possible to send notifications properly. But rest of the code will be processed.');
      console.dir(err);
    }

    // Send notifications to user - new rabbitMQ queue
  ...
  async updateOrganization(req) {
    ...
    const { updatedModel, deltaData } = await db
      .transaction(async transaction => {
        ...
        const deltaData = await UsersTeamService.processUsersTeamUpdating(org_id, OrganizationsModelProvider.getEntityName(), body, this.currentUser.id, transaction);
