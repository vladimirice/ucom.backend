...
const UsersActivityFollowRepository = require("../../users/repository/users-activity/users-activity-follow-repository");
...
class OrganizationsFetchService {
    ...
    static async findAllFollowedByUserAndProcess(userId, query) {
        const repository = OrganizationsRepository;
        const knexForList = OrganizationsRepository.getQueryBuilder();
        const knexForCount = OrganizationsRepository.getQueryBuilder();
        const { offset, limit } = QueryFilterService.addQueryParamsToKnex(query, repository, knexForList);
        UsersActivityFollowRepository.addWhereInOrgsEntityId(knexForList, OrganizationsModelProvider.getTableName(), userId);
        UsersActivityFollowRepository.addWhereInOrgsEntityId(knexForCount, OrganizationsModelProvider.getTableName(), userId);
