...
class OrganizationsResendingService {
  public static async resendOrganizations(
  ...
  private static getManyOrganizations(createdAtLessOrEqualThan: string, limit: number, offset: number) {
  ...
  private static async resendOrganizationOneByOne(manyOrganizations, printPushResponse: boolean) {
    ...
    for (const organization of manyOrganizations) {
      ...
      const signedTransaction = await OrganizationsApi.signResendOrganizations(
        organization.account_name_from,
...
export = OrganizationsResendingService;
