...
class ActivityService {
  ...
  static async userFollowsUser(userFrom, userIdTo) {
    ...
    try {
      ...
      await this.setBlockchainStatusIsError(newActivity)
    }
  ...
  static async userUpvotesComment(userFrom, commentTo) {
    ...
    const transactionResult = await models.sequelize
      .transaction(async transaction => {
        await Promise.all([
          ActivityUserCommentRepository.createNewActivity(userFrom.id, commentTo.id, activityTypeId, transaction),

          // TODO update votes counter
          // models['posts'].update({ current_vote: models.sequelize.literal('current_vote + 1') }, { where: { id: postTo.id } , transaction})
        ]);
