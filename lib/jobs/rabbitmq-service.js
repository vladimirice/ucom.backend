...
class RabbitMqService {
  ...
  static _setConnectionEventHandlers() {
    connection.on("error", function(err) {
      winston.error('RabbitMq connection error event. Lets try to reset connection. Error message is: ', err.message);
      RabbitMqService._resetChannelAndConnection();
    });

    connection.on("close", function() {
      winston.error('RabbitMq connection is closed. Lets try to reset connection.');
      RabbitMqService._resetChannelAndConnection();
    });
  }
  ...
  static _setChannelEventHandlers() {
    channel.on("error", function(err) {
      winston.error('RabbitMq channel error event. Lets try to reset channel and connection. Error message is: ', err.message);
      RabbitMqService._resetChannelAndConnection();
    });

    channel.on("close", function() {
      winston.error('RabbitMq connection is closed. Lets try to reset channel and connection.');
      RabbitMqService._resetChannelAndConnection();
    });
  }
