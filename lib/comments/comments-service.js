...
class CommentsService {
     ...
     * @private
     */
    static async addTransactionDataToBody(body, currentUser, parentModelBlockchainId, isCommentOnComment, organizationBlockchainId = null) {
     ...
     * @private
     */
    async processOrganizationAction(post, body) {
        ...
        const isTeamMember = await usersTeamRepository.isTeamMember(orgModelProvider.getEntityName(), post.organization.id, this.currentUser.id);
        if (!isTeamMember) {
            body.organization_id = null;
            return;
        }
     ...
     * @private
     */
    async createNewComment(body, parentIdInBlockchain, post, parentModel, isCommentOnComment) {
        const organizationBlockchainId = post.organization ? post.organization.blockchain_id : null;
        await CommentsService.addTransactionDataToBody(body, this.currentUser.user, parentIdInBlockchain, isCommentOnComment, organizationBlockchainId);
        ...
        const { newComment, newActivity } = await db
            ...
            const newActivity = await this.processBlockchainCommentCreation(newComment.id, body.signed_transaction, transaction, !!body.organization_id, isCommentOnComment ? newComment.parent_id : newComment.commentable_id, isCommentOnComment ? commentsModelProvider.getEntityName() : postsModelProvider.getEntityName(), eventId);
     ...
     * @private
     */
    getEventId(entityName, newModel, commentableModel) {
     ...
     * @private
     */
    async processBlockchainCommentCreation(newCommentId, signedTransaction, transaction, isOrganization, commentableId, commentableName, eventId) {
        return await usersActivityService.processCommentCreation(this.currentUser.id, newCommentId, signedTransaction, isOrganization, commentableId, commentableName, eventId, transaction);
    }
}
