...
const CommentsPostProcessor = require('./service/comments-post-processor');
...
class CommentsService {
  ...
  async findAndProcessCommentsByPostId(postId) {
    ...
    const data = this._processManyComments(dbData);
   ...
   * @private
   */
  _processManyComments(comments) {
    const processedComments = CommentsPostProcessor.processManyComments(comments, this.currentUser.id);
  ...
  async _createNewComment(body, parentIdInBlockchain, post, parentModel, isCommentOnComment) {
    ...
    const {newComment, newActivity} = await db
      .transaction(async transaction => {
        ...
        const newActivity = await this._processBlockchainCommentCreation(newComment.id, body.signed_transaction, transaction, !!body.organization_id);
   ...
   * @private
   */
  async _processBlockchainCommentCreation(newCommentId, signedTransaction, transaction, isOrganization) {
