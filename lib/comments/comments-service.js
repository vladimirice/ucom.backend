...
class CommentsService {
   ...
   * @private
   */
  async _addTransactionDataToBody(
    body,
    currentUser,
  ...
  async _createNewComment(body, parentIdInBlockchain, post, parentModel, isCommentOnComment) {
    ...
    await this._addTransactionDataToBody(
      body,
    ...
    return await db
      .transaction(async transaction => {
        ...
        await this._processBlockchainCommentCreation(newComment.id, body.signed_transaction, transaction);
  ...
  async _processBlockchainCommentCreation(newCommentId, signedTransaction, transaction) {
    const newActivity = await UsersActivityService.processCommentCreation(
      this.currentUser.id,
      newCommentId,
      signedTransaction,
      true,
