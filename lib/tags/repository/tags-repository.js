...
class TagsRepository {
    ...
    static async findManyTagsForList(requestQuery, params) {
        const res = await tags_model_1.TagDbModel.prototype.findAllTagsBy(requestQuery, params).fetchAll();
        return res.toJSON();
    }
    ...
    static async countManyTagsForList(requestQuery, params) {
        const queryBuilder = knex(TABLE_NAME).count(`${TABLE_NAME}.id AS amount`);
        tags_model_1.TagDbModel.prototype.addCurrentParamsInnerJoin(queryBuilder);
        tags_model_1.TagDbModel.prototype.addSearchWhere(queryBuilder, requestQuery);
        if (params.whereRaw) {
            // noinspection JSIgnoredPromiseFromCall
