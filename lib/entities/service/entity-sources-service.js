...
class EntitySourceService {
  ...
  static async _processOneSourceKey(entity_id, entity_name, data, key, source_group_id, transaction) {
    ...
    // TODO
    // updatedModels = this._filterUserRequest(updatedModels);
    ...
    return Repository.updateSourcesByDelta(
      entity_id,
      entity_name,
      deltaData,
      sourceData,
  ...
  // static _filterUserRequest(updatedModels) {
  //   return updatedModels.map(model => {
  //
  //   });
  //
  //   let {error, value} = Joi.validate(body, CreateUpdateValidatorSchema, {
  //     allowUnknown: true,
  //     abortEarly:   false,
  //     stripUnknown: true,
  //   });
