...
const EntitySourcesRepository = require("../repository/entity-sources-repository");
const EntitySourcesDictionary = require("../dictionary/entity-sources-dictionary");
...
class EntitySourceService {
    ...
    static async processSourcesUpdating(entityId, entityName, body, transaction) {
        for (const sourceType in sourceTypes) {
            ...
            if (DeleteAllInArrayValidator.isValueMeanDeleteAll(sources)) {
                ...
                await EntitySourcesRepository.deleteAllForOrgBySourceTypeId(entityId, entityName, sourceTypeSet.source_group_id);
                continue;
            }
        ...
        sources.forEach((source) => {
            if (source.source_type === EntitySourcesDictionary.internalType()) {
                ...
                throw new BadRequestError({
                    source_type: `Source type ${source.source_type} is not supported. Only ${EntitySourcesDictionary.internalType()} or ${EntitySourcesDictionary.externalType()}`,
                });
            }
        });
        return result;
    ...
    static getDataForSocialNetworks(parentEntityId, parentEntityName, sources, sourceSet) {
        ...
        sources.forEach((entity) => {
            result.push(Object.assign(Object.assign({}, entity), appendData));
        });
        return result;
    }
    // noinspection OverlyComplexFunctionJS
    static async processOneSourceKey(entityId, entityName, body, sourceKey, sourceGroupId, transaction) {
