...
class EntitySourceService {
   ...
   */
  static async processCreationRequest(entity_id, entity_name, body, transaction) {
  ...
  static async processSourcesUpdating(
    ...
    for (const sourceType in sourceTypes) {
      ...
      if (sources) {
        await this._processOneSourceKey(entity_id, entity_name, data, sourceType, sourceTypeSet.source_group_id, transaction)
      }
    }
  }
  ...
  static async _processOneSourceKey(entity_id, entity_name, data, key, source_group_id, transaction) {
    ...
    if (source_group_id === SOURCE_GROUP__SOCIAL_NETWORKS) {
      UpdateManyToManyHelper.filterDeltaDataBeforeSave(deltaData, CreateEntitySourceSchema, UpdateEntitySourceSchema);
    } else {
      this._processExternalTextDataBeforeSave(deltaData.added);
      this._processExternalTextDataBeforeSave(deltaData.changed);
    }
  ...
  static _processExternalTextDataBeforeSave(models) {
