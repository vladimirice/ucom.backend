...
class EntityNotificationsRepository {
   ...
   */
  static async createOrgBoardInvitationForOneUser(activity) {
    const data = {
        title:                  '',
        description:            '',
   ...
   * @return {Promise<data>}
   */
  static async createUserFollowsOrg(activity, orgMemberIdToReceive) {
    const data = {
   ...
   * @return {Promise<data>}
   */
  static async createUserFollowsOtherUser(activity) {
    const data = {
  ...
  static _getAttributesToSelectForList() {
    ...
    orgAttributes.forEach(attribute => {
      toSelect.push(`org_is_target.${attribute} AS org_is_target__${attribute}`)
    });
  ...
  static _arrangeResponse(dbData) {
    ...
    for (let i = 0; i < dbData.length; i++) {
      ...
      let keyFromDbTargetEntity;
      let keyForDataTargetEntity;
      ...
      switch(notification.entity_notifications__event_id) {
        ...
        case EventIdDictionary.getUserFollowsOrg():
          ...
          keyFromDbTargetEntity = 'org_is_target__';
          keyForDataTargetEntity = 'organization';
      ...
      if (keyFromDbTargetEntity) {
        packedOne.target_entity = {};
        packedOne.target_entity[keyForDataTargetEntity] = {}
      }
      ...
      for (const field in notification) {
        ...
        if (field.includes(keyFromDbTargetEntity)) {
          const fieldName = field.replace(keyFromDbTargetEntity, '');
          packedOne.target_entity[keyForDataTargetEntity][fieldName] = notification[field];
        }
      }
