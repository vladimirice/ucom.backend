...
const EntityStatsCurrentModel = models['entity_stats_current'];
...
class PostsRepository {
   ...
   *
   * @param {Object|null} queryParameters
   * @param {boolean} isDeltaStats - not optimal but quick solution
   * @return {Promise<any[]>}
   */
  static async findAllPosts(queryParameters = {}, isDeltaStats = false) {
    ...
    // It is not required yet to show this param - only order by if such order by is mentioned
    if (isDeltaStats) {
      include.push({
