...
const AirdropsFetchRepository = require("../../airdrops/repository/airdrops-fetch-repository");
...
class PostsFetchService {
    ...
    static async findOnePostByIdAndProcessV2(postId, currentUserId, commentsQuery, tryAirdropOffer = false) {
    ...
    static async addDataForGithubAirdropOffer(post) {
        ...
        state.tokens.forEach((item) => {
            tokens.push({
                ...
                amount_left: (item.amount_claim - 50000) / (10 ** item.precision),
                symbol: item.symbol,
            });
        });
