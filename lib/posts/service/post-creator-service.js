...
class PostCreatorService {
  ...
  static processEntityImagesWhileCreation(body) {
    console.log('Process entity images body JSON: ', JSON.stringify(body, null, 2));
  ...
  static checkPostEntityImages(model) {
    if (!model.entity_images) {
      throw new BadRequestError('Model musth contain entity_images field')
    }
    ...
    for (let i = 0; i < model.entity_images.article_title.length; i++) {
