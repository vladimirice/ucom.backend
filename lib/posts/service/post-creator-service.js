...
class PostCreatorService {
    ...
    static async processNewPostCreation(req, eventId = null, currentUser) {
        ...
        const { newPost, newActivity } = await models.sequelize
            ...
            const activity = await this.createNewActivity(model, signedTransaction, currentUser.id, eventId, transaction);
            return {
                newPost: model,
    ...
    static async processRepostCreation(givenBody, postId, currentUser) {
        ...
        const eventId = ucom_libs_common_1.EventsIdsDictionary.getRepostEventId(parentPost.organization_id);
