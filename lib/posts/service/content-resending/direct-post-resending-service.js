...
class DirectPostResendingService {
    static async resendDirectPostsForUser(createdAtLessOrEqualThan, limit, printPushResponse = false, offset = 0) {
    ...
    static async resendDirectPostsForOrganizations(createdAtLessOrEqualThan, limit, printPushResponse = false, offset = 0) {
        EosApi.initBlockchainLibraries();
    ...
    static getManyDirectPostsForUser(createdAtLessOrEqualThan, limit, offset) {
        return knex(`${PostsModelProvider.getTableName()} AS p`)
            ...
            .where('p.post_type_id', ucom_libs_common_1.ContentTypesDictionary.getTypeDirectPost())
            .where('p.created_at', '<=', createdAtLessOrEqualThan)
            .orderBy('p.id', 'ASC')
            .limit(limit)
            .offset(offset);
    }
    static getManyDirectPostsForOrganization(createdAtLessOrEqualThan, limit, offset) {
        return knex(`${PostsModelProvider.getTableName()} AS p`)
            .select([
            'p.blockchain_id as blockchain_id',
            'p.description as description',
    ...
    static async resendPostsOneByOneForUser(manyPosts, printPushResponse) {
        ...
        for (const post of manyPosts) {
            ...
            const signedTransaction = await PublicationsApi.signResendDirectPostsToAccount(post.account_name_from, EosApi.getHistoricalSenderPrivateKey(), post.account_name_to, post, post.blockchain_id);
    ...
    static async resendPostsOneByOneForOrganizations(manyPosts, printPushResponse) {
        let processedCount = 0;
        for (const post of manyPosts) {
            if (processedCount % 100 === 0) {
                console.log(`Current processed count is: ${processedCount}`);
            ...
            post.created_at = moment(post.created_at).utc().format();
            post.updated_at = moment(post.updated_at).utc().format();
            const signedTransaction = await PublicationsApi.signResendDirectPostsToOrganization(post.account_name_from, EosApi.getHistoricalSenderPrivateKey(), post.organization_id_to, post, post.blockchain_id);
            const pushingResponse = await EosClient.pushTransaction(signedTransaction);
            if (printPushResponse) {
                console.log(`Transaction id: ${pushingResponse.transaction_id}`);
                console.dir(JSON.stringify(pushingResponse.processed.action_traces[0].act.data));
...
module.exports = DirectPostResendingService;
