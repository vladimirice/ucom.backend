...
const EntityStatsCurrentModel = models['entity_stats_current']; // TODO hardcode
...
class PostService {
  ...
  async findAll(query) {
    ...
    // hardcode
    // TODO - temp solution - interface for frontend
    if (query.sort_by) {
      query.sort_by = query.sort_by.replace('created_at_date', 'created_at');
      query.sort_by = query.sort_by.replace('current_rate_delta_daily', 'current_rate');
    }

    let isUpvoteDelta = false;
    if (query.sort_by === '-rate_delta') {
      // query.sort_by += ',-current_vote';
      query.sort_by = '-current_rate';
      isUpvoteDelta = false;
    }

    let params = QueryFilterService.getQueryParameters(query, orderByRelationMap, PostsRepository.getAllowedOrderBy());

    const [posts, totalAmount] = await Promise.all([
      PostsRepository.findAllPosts(params, isUpvoteDelta),
    ...
    const data = ApiPostProcessor.processManyPosts(posts, userId, currentUserActivity);
