...
class PostService {
  ...
  async updateAuthorPost(post_id, user_id, params) {
    ...
    if (PostService.isDirectPost(updatedPost)) {
      newPostJson = await this.findOneByIdAndProcess(updatedPost.id);
    } else {
      newPostJson = updatedPost.toJSON();
    }
  ...
  async processNewPostCreationForUser(req) {
    if (req.post_type_id !== ContentTypeDictionary.getTypeDirectPost()) {
    ...
    req.body.entity_name_for = req.user_id;
  ...
  async processNewPostCreation(req) {
    ...
    if (PostService.isDirectPost(newPost)) {
      newPostJson = await this.findOneByIdAndProcess(newPost.id);
    } else {
      newPostJson = newPost.toJSON();
    }
