...
class PostService {
  ...
  async findAll(query) {
    ...
    const postsTotalAmount = await PostsRepository.countAllPosts(params);
    ...
    let metadata = {
      ...
      'has_more':     params['offset'] + params['limit'] < postsTotalAmount,
    };
  ...
  async findAndProcessAllForUserWallFeed(userId) {
    ...
    const posts = await UsersFeedRepository.findAllForUserWallFeed(userId);
    const currentUserActivity = await this._getCurrentUserActivity(posts);
  ...
  async findAndProcessAllForMyselfNewsFeed() {
    ...
    const posts = await UsersFeedRepository.findAllForUserNewsFeed(userId, usersIds, orgIds);
    const currentUserActivity = await this._getCurrentUserActivity(posts);
