...
class PostService {
  ...
  async processNewPostCreation(req) {
    ...
    if (!body.organization_id) {
      ...
      if (!orgBlockchainId) {
        throw new AppError(`There is no orgBlockchainId for org with ID ${+body.organization_id}`)
      }
    }
    ...
    return await models.sequelize
      .transaction(async transaction => {
        const newPost = this._createPostByPostType(postTypeId, body, transaction);
        const newActivity = this._createNewActivity(newPost, body.signed_transaction, this.currentUser.getId());
   ...
   * @private
   */
  async _createNewActivity(newPost, signedTransaction, currentUserId) {
    let newActivity;
  ...
  static async _addSignedTransactionDetailsToBody(body, user, postTypeId, organizationBlockchainId = null) {
    ...
    if (organizationBlockchainId) {
      ...
      body.signed_transaction = TransactionFactory._getSignedOrganizationCreatesContent(
        user.account_name,
        user.private_key,
      ...
      // noinspection JSAccessibilityCheck
      body.signed_transaction = TransactionFactory._userHimselfCreatesPost(
        user.account_name,
        user.private_key,
        body.blockchain_id,
        postTypeId
