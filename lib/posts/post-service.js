...
class PostService {
  ...
  async findOneByIdAndProcess(postId) {
    ...
    const processedPost = PostService.processOneAfterQuery(post, this.currentUser.id, activityData);
    if (processedPost.organization_id) {
      // TODO Fetch all at once by JOIN
      const organization = await OrganizationRepositories.Main.findOneByIdForPreview(post.organization_id);
      OrganizationPostProcessor.processOneOrg(organization);

      processedPost.organization = organization;
    }
