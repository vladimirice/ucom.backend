...
class PostService {
  ...
  static async updateAuthorPost(post_id, user_id, params) {
    ...
    if (postToUpdate.post_type_id === PostTypeDictionary.getTypeOffer() && params['post_users_team']) {
    ...
    const jobPayload = PostJobSerializer.getPostDataToCreateJob(updatedPost);
  ...
  static async createNew(req) {
    ...
    await this._makeOrganizationRelatedChecks(body, user);
    ...
    // TODO - blockchain ID must be created on frontend - separately
    let newPost;
    switch (postTypeId) {
      case PostTypeDictionary.getTypeMediaPost():
        newPost = await PostsRepository.createNewPost(body, user);
        break;
      case PostTypeDictionary.getTypeOffer():
        newPost = await PostRepositories.PostOffer.createNewOffer(body, user);
        break;
      default:
        throw new BadRequestError({
    ...
    // call blockchain and activity

    // TODO - wrap in database transaction
    if (!body.organization_id) {
      console.log('No org id. Lets treat as regular creation');
      // Legacy table and legacy way to create new post blockchain transactions
      await this._createPostWithoutOrganization(user, newPost);
      ...
      console.log('On behalf of organization. Lets treat as post from organization');
      console.log('organization_id: ', +body.organization_id);
      // TODO - sign on frontend
      const orgBlockchainId = await OrganizationRepositories.Main.findBlockchainIdById(+body.organization_id);


      console.log('orgBlockchainId: ', orgBlockchainId);
  ...
  /**
   * @deprecated - legacy way to create activity and transactions. Should be replaced
   * @param {Object} user
   * @param {Object} newPost
   * @return {Promise<void>}
   * @private
   */
  static async _createPostWithoutOrganization(user, newPost) {
  ...
  async findAllByAuthor(userId) {
    ...
    return await PostService.processAllAfterQuery(posts, currentUserId);
  }
  ...
  static async addMyselfData(model, currentUserId, userActivityData) {
  ...
  static async _addOrganizationPreviewData(model) {
    ...
    const organization = await OrganizationRepositories.Main.findOneByIdForPreview(model.organization_id);
    OrganizationPostProcessor.processOneOrg(organization);

    model.organization = organization;
  }

  // noinspection FunctionWithInconsistentReturnsJS
  static async processOneAfterQuery (model, currentUserId, userActivityData = null) {
    if (!model) {
      return;
    }
    ...
    await this.addMyselfData(model, currentUserId, userActivityData);
   ...
   * @returns {Array}
   */
  static async processAllAfterQuery(models, currentUserId) {
    let result = [];

    for (let i = 0; i < models.length; i++) {
      const model = models[i];
      const data = await PostService.processOneAfterQuery(model, currentUserId);
   ...
   * @private
   */
  static async _makeOrganizationRelatedChecks(body, user) {
    if (!body.organization_id) {
      return;
    }
   ...
   * @private
   */
  static async _checkCreationBehalfPermissions(user_id, organization_id = null) {
