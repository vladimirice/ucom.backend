...
class PostService {
  ...
  async processRepostCreation(givenBody, postId) {
    ...
    await EosTransactionService.appendSignedUserCreatesRepost(body, this.currentUser.user, parentPost.blockchain_id);
    ...
    const { newPost, newActivity } = await models.sequelize
      .transaction(async transaction => {
        ...
        const newActivity = await this._createNewActivityForRepost(
          newPost,
   ...
   * @private
   */
  async _createNewActivityForRepost(newPost, signedTransaction, currentUserId, eventId = null, transaction = null) {
    let newActivity;
