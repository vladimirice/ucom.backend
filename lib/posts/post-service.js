...
class PostService {
  ...
  async findAll() {
    ...
    const posts = await PostsRepository.findAllPosts(false);
    const currentUserId = this.currentUser.getCurrentUserId();
  ...
  static processOneAfterQuery (model, currentUserId) {
    ...
    if (model['comments']) {
      model.comments.sort((a, b) => {
        let aString = a.path;
        let bString = b.path;

        // Считаем разницу в длине
        const lengthDiff = Math.abs(aString.length - bString.length);
