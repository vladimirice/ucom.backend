...
class PostService {
    ...
    static async updateAuthorPost(postId, userId, params, currentUser) {
        ...
        const signedTransaction = params.signed_transaction || '';
        // #task #optimization
        ...
        const { updatedPost, newActivity } = await models.sequelize.transaction(async (transaction) => {
            ...
            const activity = await UserActivityService.processPostIsUpdated(updated, currentUserId, transaction, signedTransaction);
            return {
                updatedPost: updated,
    ...
    static async processNewDirectPostCreationForUser(req, currentUser) {
        ...
        await eosTransactionService.appendSignedUserCreatesDirectPostForOtherUser(req.body, currentUser, accountNameTo);
    ...
    static async processNewDirectPostCreationForOrg(req, currentUser) {
        ...
        await eosTransactionService.appendSignedUserCreatesDirectPostForOrg(req.body, currentUser, orgBlockchainId);
        return PostCreatorService.processNewPostCreation(req, eventId, currentUser);
    }
