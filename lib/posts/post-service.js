...
class PostService {
  ...
  static async _checkBehalfPermissions(user_id, organization_id = null) {
  ...
  static async createNew(req) {
    ...
    await this._checkBehalfPermissions(user.id, body.organization_id);
  ...
  async findAllByOrganization(organization_id) {
    const currentUserId = this.currentUser.getCurrentUserId();
    const posts = await PostsRepository.findAllByAuthorAndOrganization(organization_id, false);
