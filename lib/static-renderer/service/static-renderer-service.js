...
class StaticRendererService {
    ...
    static async getHtml(host, originalUrl) {
        ...
        try {
            ...
            const mainImageUrl = this.extractMainImageUrl(model);
            const metaTagsValues = {
                '{{og_url_value}}': `https://${host}${originalUrl}`,
            ...
            const metaTagsFragment = this.getMetaTagsFragment(metaTagsValues);
            if (!frontendTemplate.includes(CONTENT_META_TAGS_PLACEHOLDER)) {
                throw new Error(`There is no content meta tags placeholder inside frontendTemplate. Filename is: ${frontendMainHtml}. Searching for ${CONTENT_META_TAGS_PLACEHOLDER}`);
            }
     ...
     * @private
     */
    static extractMainImageUrl(model) {
     ...
     * @private
     */
    static getMetaTagsFragment(metaTagsValues) {
