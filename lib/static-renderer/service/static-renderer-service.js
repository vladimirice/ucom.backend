...
class StaticRendererService {
  ...
  static async getHtml(host, originalUrl) {
    ...
    try {
      frontendTemplate = fs.readFileSync(frontendConfig.main_html_filename).toString();
      ...
      if (!frontendTemplate.includes(CONTENT_META_TAGS_PLACEHOLDER)) {
        throw new Error(
          `There is no content meta tags placeholder inside frontendTemplate. Filename is: ${frontendConfig.main_html_filename}. Searching for ${CONTENT_META_TAGS_PLACEHOLDER}`
        );
      }
