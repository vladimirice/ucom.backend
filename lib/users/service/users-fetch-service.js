...
const UsersActivityVoteRepository = require("../repository/users-activity/users-activity-vote-repository");
const UsersActivityEventsViewRepository = require("../repository/users-activity/users-activity-events-view-repository");
class UsersFetchService {
    static async findOneAndProcessFully(userId, currentUserId) {
        ...
        userJson.views_count =
            await UsersActivityEventsViewRepository.getViewsCountForEntity(userJson.id, ucom_libs_common_1.EntityNames.USERS);
        return userJson;
    }
    ...
    static async findOneContentVotingUsers(query, currentUserId) {
        const repository = UsersRepository;
        const params = QueryFilterService.getQueryParametersWithRepository(query, repository, true, false, true);
