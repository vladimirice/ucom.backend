...
class UserActivityService {
     ...
     * @return {Promise<Object>}
     */
    static async processUserFollowsOrUnfollowsUser(activityTypeId, signedTransaction, currentUserId, userIdTo, transaction) {
    ...
    static async createAndSignOrganizationCreationTransaction(userFrom, newOrganizationBlockchainId) {
        ...
        return await TransactionFactory.createSignedUserCreatesOrganization(userFrom.account_name, userFrom.private_key, newOrganizationBlockchainId);
    }
    /**
     *
     * @param {Object} userFrom
    ...
    static async userFollowOrUnfollowUser(userFrom, userIdTo, activityTypeId, body) {
        ...
        const activity = await db
            ...
            return await this.processUserFollowsOrUnfollowsUser(activityTypeId, signed, userFrom.id, userIdTo, transaction);
        });
        await this.sendPayloadToRabbit(activity);
        return true;
    }
    static async getSignedFollowTransaction(userFrom, userToAccountName, activityTypeId) {
        return await TransactionFactory._getSignedUserToUser(userFrom.account_name, userFrom.private_key, userToAccountName, activityTypeId);
    }
    /**
     *
