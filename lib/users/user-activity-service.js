...
const ACTIVITY_TYPE__CANCEL_NODE_UPVOTING_NODE = 20;
...
class UserActivityService {
  ...
  static async processUserVotesChangingForBlockProducers(userId, blockchainNodeIds, transaction) {
    ...
    const data = [];
    for (let i = 0; i < blockchainNodeIds.length; i++) {
      data.push({
    ...
    return await UsersActivityRepository.bulkCreateNewActivity(data, transaction);
  }
