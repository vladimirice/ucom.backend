  ...
  static async _userFollowOrUnfollowUser(userFrom, userIdTo, activityTypeId, body = null) {
    ...
    const activity = await db
      .transaction(async transaction => {
        ...
        let signed = null;
        if (body) {
          console.log('signed is get from request body');
          signed = JSON.parse(body);
          console.log('success, signed is ', signed);
        } else {
          // noinspection JSUnresolvedFunction
          console.log('signed is get from backend');
          signed = await TransactionFactory._getSignedUserToUser(
            userFrom.account_name,
            userFrom.private_key,
            userToAccountName,
            activityTypeId
