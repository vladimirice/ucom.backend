...
const EosAuth = require('../eos/eos-auth');
...
class AuthValidator {
  ...
  static async validateNewAccountName(account_name) {
    ...
    if (!account_name) {
      throw new Error('Account name parameter is required');
    }
    ...
    if (premiumAccounts.indexOf(account_name) === -1
      ...
      throw new Error('Account name must contain only a-z or 1-5 and must have exactly 12 symbols length');
    }

    const user = await UsersService.findOneByAccountName(account_name);
    if (user) {
      throw new Error('That account name is taken. Try another');
    }
