...
class UpdateManyToManyHelper {
  ...
  static async updateSourcesByDelta(model, deltaData, appendDataForNew, transaction) {
    const creationPromises = this._getCreationPromises(model, deltaData, appendDataForNew, transaction);
    const updatePromises = this._getUpdatePromises(model, deltaData, transaction);
    const deletionPromises = this._getDeletionPromises(model, deltaData, transaction);
  ...
  static filterDeltaDataBeforeSave(deltaData, creationValidationSchema, updatingValidationSchema) {
    deltaData.added   = this._filterManySources(deltaData.added, creationValidationSchema);
    deltaData.changed = this._filterManySources(deltaData.changed, updatingValidationSchema);
  }
   ...
   * @private
   */
  static _getCreationPromises(model, deltaData, appendDataForNew, transaction) {
    let promises = [];
   ...
   * @private
   */
  static _getUpdatePromises(model, deltaData, transaction) {
    let promises = [];
   ...
   * @private
   */
  static _getDeletionPromises(model, deltaData, transaction) {
    let promises = [];
   ...
   * @private
   */
  static _filterManySources(updatedModels, joiSchema) {
    return updatedModels.map(source => {
      return this._filterOneSource(source, joiSchema);
    });
  }
   ...
   * @private
   */
  static _filterOneSource(source, joiSchema) {
    let {error, value} = Joi.validate(source, joiSchema, {
      allowUnknown: true,
      abortEarly:   false,
      stripUnknown: true,
    });
