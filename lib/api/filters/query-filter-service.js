...
class QueryFilterService {
   ...
   * @returns {Object}
   */
  static getQueryParameters(query, orderByRelationMap = {}, allowedSortBy = []) {
    let params = {};
   ...
   * @returns {void}
   */
  static _setOrderBy(query, params, orderByRelationMap, allowedSortBy = []) {
    if (!query.sort_by) {
      return;
    }
    ...
    query['sort_by'].split(',').forEach(value => {
      ...
      if (allowedSortBy.length > 0 && !(~allowedSortBy.indexOf(valueToSort))) {
        throw new BadRequestError(
          `sort_by field ${valueToSort} is not supported. Supported fields are: ${allowedSortBy.join(', ')}`
        );
      }
