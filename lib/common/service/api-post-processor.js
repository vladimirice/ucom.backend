...
class ApiPostProcessor {
  ...
  * @param {Object} models
  */
  static processManyNotifications(models) {
    models.forEach(model => {
      this.processOneNotification(model);
    })
  }
   ...
   * @param {Object} model
   */
  static processOneNotification(model) {
    // TODO - only for board notification prompt - not flexible

    model.data = {};
    model.target_entity = {};

    model.data.organization = model.organization;
    model.target_entity.User = model.User;
