...
class ApiPostProcessor {
  ...
  */
  ...
  static processOnePostForList(post, currentUserId = null, userActivity = null) {
    ...
    // TODO return is not required here
    return post;
  }
  ...
  static _addMyselfDataForPost(model, currentUserId, userPostActivity, orgTeamMembers = []) {
    ...
    let join = false; // not supported, suspended
    let organization_member = false;
    let repost_available    = true;
    ...
    if (userPostActivity) {
      if (userPostActivity.voting) {
        myselfVote = userPostActivity.voting === InteractionTypeDictionary.getUpvoteId() ? 'upvote' : 'downvote';
      }

      repost_available = !userPostActivity.repost;
    }
