...
class ApiPostProcessor {
  ...
  static processManyPosts(posts, currentUserId, currentUserActivity) {
    ...
    for (let i = 0; i < posts.length; i++) {
      const post = posts[i];
      const data = this.processOnePostFully(post, currentUserId, currentUserActivity);
  ...
  static processOnePostFully(post, currentUserId, currentUserActivity) {
    ...
    UsersPostProcessor.processModelAuthor(post, currentUserId, currentUserActivity);
   ...
   */
  static addMyselfDataForPost(model, currentUserId, userPostActivity) {
    if (!currentUserId) {
      return;
    }
