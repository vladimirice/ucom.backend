...
class RepositoryHelper {
    ...
    static async doesExistByQueryBuilder(queryBuilder) {
    ...
    static convertStringFieldsToNumbers(model, fields, fieldsToDisallowZero = []) {
        for (const field of fields) {
            model[field] = NumbersHelper.processFieldToBeNumeric(model[field], field, 20, fieldsToDisallowZero.includes(field), false);
        }
    }
