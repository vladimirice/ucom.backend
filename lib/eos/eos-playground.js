...
const UserUser = require('../../lib/activity/user-user');
...
class EosService {
  ...
  static sendUserToUser(senderData, recipientData, interactionTypeId) {
    const eos = EosPlayground({
      keyProvider: [
        senderData.activePk,
      ],
  ...
  static async createAccount(accountName, brainkey) {
    ...
    return await eos.transaction(tr => {
      tr.newaccount({
        creator: eosAccounts['creator_account_name'],
        name: accountName,
      ...
      tr.buyrambytes({
        payer: eosAccounts['creator_account_name'],
        receiver: accountName,
        bytes: 8192
      });

      tr.delegatebw({
        from: eosAccounts['creator_account_name'],
        receiver: accountName,
