...
class ImportanceEventService {
    ...
    static async findStatsData() {
        ...
        return await Promise.all([
            entityEventParamRepository.findLastRowsGroupedByEntity(`event_type = ${eventType} AND entity_name = '${postsModelProvider.getEntityName()}'`),
            entityEventParamRepository.findLastRowsGroupedByEntity(`"event_type" = ${eventType} AND entity_name = '${postsModelProvider.getEntityName()}' AND created_at < '${createdAtAsString}'`),
        ]);
    }
    /**
     *
     ...
     * @private
     */
    static prepareBasicToProcess(lastData) {
     ...
     * @private
     */
    static async addEntityIdToData(toProcess, entityNameToBlockchainId) {
        // Delta for posts only
        const postIds = await postsRepository.findIdsByBlockchainIds(entityNameToBlockchainId[postsModelProvider.getEntityName()]);
        if (postIds.length === 0) {
     ...
     * @private
     */
    static addRateDelta(toProcess, lastOfGivenDateData) {
