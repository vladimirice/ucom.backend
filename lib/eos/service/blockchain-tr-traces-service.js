...
class BlockchainTrTracesService {
  ...
  static async syncMongoDbAndPostgres() {
    ...
    for(let i = 0; i < TRANSACTION_TYPES.length; i++) {
      console.log(`Lets begin sync for ${TRANSACTION_TYPES[i]}`);
      const hrstart = process.hrtime();
      await this._findAndSyncByTrType(TRANSACTION_TYPES[i]);
  ...
  static async _findAndSyncByTrType(trType) {
    ...
    const existedIds = await BlockchainTrTracesRepository.findAllExistingTrIds(trType);
    ...
    do {
      ...
      const processedData = this._processDocsForStorage(docs, processor, existedIds);

      for (let i = 0; i < processedData.length; i++) {
        const current = processedData[i];

        await BlockchainTrTracesRepository.insertOneTrace(current);
      }
