...
class TrTracesProcessor {
  ...
  static processOneTrByType(doc, trType) {
    this._checkDocCommonFields(doc);

    const processor = TrTracesProcessor._getProcessor(trType);
  ...
  static _processTrTransfer(doc) {
    ...
    if (actionTraces.length !== 1 || !actionTraces[0]) {
      throw new TrTracesProcessorError(
        'Action traces should has only one element',
        actionTraces
      );
    }
    ...
    if (!traceData) {
      throw new TrTracesProcessorError(
        'There is no data inside act',
        actionTraces[0]
      );
    }

    TrTracesProcessor._checkRequiredFields(tracedDataRequiredFields, traceData);
    ...
    return TrTracesProcessor._getProcessedObj(
  ...
  static _processTrStake(doc) {
    ...
    return TrTracesProcessor._processTrStakingOrUnstakingRequest(
      doc,
      trType,
      cpuQuantityField,
      netQuantityField,
   ...
   * @private
   */
  static _processTrStakingOrUnstakingRequest(
    doc,
    trType,
    cpuQuantityField,
    netQuantityField,
    ...
    if (actOne.name !== skipWord) {
      ...
      TrTracesProcessor._checkRequiredFields(tracedDataRequiredFields, traceDataOne);

      traceOneCpuAmount = TrTracesProcessor.getTokensAmountFromString(traceDataOne[cpuQuantityField]);
      traceOneNetAmount = TrTracesProcessor.getTokensAmountFromString(traceDataOne[netQuantityField]);
    ...
    if (actTwo && actTwo.name !== skipWord) {
      ...
      TrTracesProcessor._checkRequiredFields(tracedDataRequiredFields, traceDataTwo);

      traceTwoCpuAmount = TrTracesProcessor.getTokensAmountFromString(traceDataTwo[cpuQuantityField]);
      traceTwoNetAmount = TrTracesProcessor.getTokensAmountFromString(traceDataTwo[netQuantityField]);
  ...
  static _processTrBuyRamBytes(doc) {
    ...
    if (actionTraces.length !== 1 || !actionTraces[0]) {
      throw new TrTracesProcessorError(
        `actionTraces.length !== 1 || !actionTraces[0]`,
        actionTraces
      );
    }
    ...
    if (!inlineTraces || inlineTraces.length !== 2 || inlineTraces.length === 0) {
      throw new TrTracesProcessorError(
        `!inlineTraces || inlineTraces.length !== 2 || inlineTraces.length === 0`,
        actionTraces[0]
      );
    }
    ...
    if (!inlineTraces[0] || !inlineTraces[0].act || !inlineTraces[0].act.data || !inlineTraces[0].act.data.quantity) {
      throw new TrTracesProcessorError(
        `!inlineTraces[0] || !inlineTraces[0].act || !inlineTraces[0].act.data || !inlineTraces[0].act.data.quantity`,
        actionTraces[0]
      );
    }
    ...
    if (!inlineTraces[1] || !inlineTraces[1].act || !inlineTraces[1].act.data || !inlineTraces[1].act.data.quantity) {
      throw new TrTracesProcessorError(
        `!inlineTraces[1] || !inlineTraces[1].act || !inlineTraces[1].act.data || !inlineTraces[1].act.data.quantity`,
        actionTraces[0]
      );
    }
    ...
    if (traceData.payer !== traceData.receiver) {
      throw new TrTracesProcessorError(
        `traceData.payer !== traceData.receiver`,
        traceData
      );
    }
  ...
  static _processTrSellRamBytes(doc) {
    ...
    if (!inlineTraces[0] || !inlineTraces[0].act || !inlineTraces[0].act.data || !inlineTraces[0].act.data.quantity || !inlineTraces[0].act.data.memo) {
      throw new TrTracesProcessorError(
        '!inlineTraces[0] || !inlineTraces[0].act || !inlineTraces[0].act.data || !inlineTraces[0].act.data.quantity || !inlineTraces[0].act.data.memo',
        actionTraces[0]
      );
    }
    ...
    if (!inlineTraces[1] || !inlineTraces[1].act || !inlineTraces[1].act.data || !inlineTraces[1].act.data.quantity || !inlineTraces[1].act.data.memo) {
      throw new TrTracesProcessorError(
        '!inlineTraces[1] || !inlineTraces[1].act || !inlineTraces[1].act.data || !inlineTraces[1].act.data.quantity || !inlineTraces[1].act.data.memo',
        actionTraces[0]
      );
    }
  ...
  static _processStakeWithUnstake(doc) {
    ...
    if (actNames[0] === actNames[1] && actNames[1] === 'delegatebw') {
    ...
    if (!~actNames.indexOf('delegatebw') || !~actNames.indexOf('undelegatebw')) {
      throw new TrTracesProcessorError('_processStakeWithUnstake. There is no stake or unstake trace inside', doc.action_traces);
    }
    ...
    if (!stakeData || !unstakeData) {
      throw new TrTracesProcessorError(
        '_processStakeWithUnstake. There is no stake or unstake trace inside',
        traces
      );
    }
  ...
  static _processTrClaimEmission(doc) {
    ...
    if (!actionTrace) {
      throw new TrTracesProcessorError(
        'There is no trace with name withdrawal',
        doc.action_traces
      );
    }
    ...
    if (!inlineTrace) {
      throw new TrTracesProcessorError(
        'There is no inline trace with act name issue',
        doc.action_traces[0]
      );
    }
  ...
  static _checkDocCommonFields(doc) {
    ...
    if (!doc.block_data || Object.keys(doc.block_data).length === 0) {
      throw new TrTracesProcessorError(`There is no block data inside doc or block data is empty`);
    }

    TrTracesProcessor._checkRequiredFields(docRequiredFields, doc);
  }
