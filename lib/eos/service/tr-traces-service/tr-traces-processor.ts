...
class TrTracesProcessor {
   ...
   * @private
   */
  private static processTrTransfer(doc) {
    ...
    return TrTracesProcessor.getProcessedObj(
   ...
   * @private
   */
  private static processTrStake(doc) {
    ...
    return TrTracesProcessor.processTrStakingOrUnstakingRequest(
      doc,
      trType,
      cpuQuantityField,
      netQuantityField,
   ...
   * @private
   */
  private static processTrUnstakingRequest(doc) {
   ...
   * @private
   */
  private static processTrStakingOrUnstakingRequest(
    doc,
    trType,
    cpuQuantityField,
    netQuantityField,
    ...
    if (traceOneAccountNameFrom && traceTwoAccountNameFrom) {
      if (traceOneAccountNameFrom !== traceTwoAccountNameFrom) {
        throw new TrTracesProcessorError(
          'Account name from (from) field must be equal in both traces',
        );
      }
    }
    ...
    if (traceOneAccountNameTo && traceTwoAccountNameTo) {
      if (traceOneAccountNameTo !== traceTwoAccountNameTo) {
        throw new TrTracesProcessorError(
          'Account name to (receiver) field must be equal in both traces',
        );
      }
    }
    ...
    if (accountNameFrom !== accountNameTo) {
      throw new TrTracesProcessorError(
        'Account name to (receiver) field must be equal to account name from (from)',
      );
    }
   ...
   * @private
   */
  private static processTrVoteForBp(doc) {
    const tracedDataRequiredFields = [
      'producers',
      'voter',
    ];
    ...
    if (actionTraces.length !== 1 || !actionTraces[0]) {
      throw new TrTracesProcessorError(
        'Action traces length !== 1 or there is no action trace with index 0',
        actionTraces,
      );
    }
    ...
    if (!act) {
      throw new TrTracesProcessorError(
        'There is no act field inside action traces',
        actionTraces[0],
      );
    }
   ...
   * @private
   */
  private static processTrBuyRamBytes(doc) {
    const actionTraces = doc.action_traces;
   ...
   * @private
   */
  private static processTrSellRamBytes(doc) {
    const actionTraces = doc.action_traces;
   ...
   * @private
   */
  private static processStakeWithUnstake(doc) {
    ...
    if (!stakeData || !unstakeData) {
      throw new TrTracesProcessorError(
        'processStakeWithUnstake. There is no stake or unstake trace inside',
        traces,
      );
    }
   ...
   * @private
   */
  private static processTrClaimEmission(doc) {
   ...
   * @private
   */
  private static processNewAccount(doc) {
   ...
   * @private
   */
  private static checkDocCommonFields(doc) {
   ...
   * @private
   */
  private static checkRequiredFields(expected, actual) {
   ...
   * @private
   */
  private static getProcessedObj(trType, trProcessedData, doc, accountNameFrom, accountNameTo = null, memo = '') {
