...
class BlockchainCacheService {
    ...
    static prepareUsersActivityPromises(votersToProcess, blockchainNodesType) {
        ...
        for (const voterId in votersToProcess) {
            ...
            const { eventIdUp, eventIdDown } = NotificationsEventIdDictionary.getUpDownEventsByBlockchainNodesType(blockchainNodesType);
            if (producersToCreate.length > 0) {
                promises.push(usersActivityService.processUserVotesChangingForBlockProducers(voterData.user_id, producersToCreate, null, eventIdUp));
            }
            if (producersToCancel.length > 0) {
                promises.push(usersActivityService.processUserCancelVotesForBlockProducers(voterData.user_id, producersToCancel, null, eventIdDown));
            }
        }
        return promises;
    }
