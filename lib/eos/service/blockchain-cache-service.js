...
class BlockchainCacheService {
    static async updateBlockchainNodesByBlockchain() {
        ...
        const votersToProcess = this.prepareVotersToProcess(userAccountNameToId, blockchainTitleToId, currentActivities, _.cloneDeep(voters));
        ...
        for (const voterAccountName in voters) {
            ...
            for (let i = 0; i < voter.producers.length; i += 1) {
                ...
                if (!blockchainTitleToId[producerTitle]) {
                    WorkerLogger.error(`There is node title ${producerTitle} which is not present in db. Probably it is deleted. Skipped... Db set is: ${JSON.stringify(blockchainTitleToId)}`);
                    continue;
                }
