...
class BlockchainApiFetchService {
     ...
     * @private
     */
    static async getApiDbData(queryParams, userId) {
        let votedNodes = [];
        let dataObjects = [];
        if (userId) {
     ...
     * @private
     */
    static getDataForApiResponse(dataObjects, votedNodes, myselfBpVoteFilter, userId) {
        const data = [];
     ...
     * @param {Object} query
     */
    static setWhereByRequestQuery(queryParams, query) {
        if (query.search) {
