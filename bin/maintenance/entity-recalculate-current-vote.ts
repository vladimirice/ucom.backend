...
async function updateCurrentVote(entities: votedEntitySet, tableName: string) {
  let promises: any = [];
  ...
  for (const entityId in entities) {
    ...
    const sql =
      `UPDATE ${tableName} SET current_vote = ${+entity.current_vote} WHERE id = ${+entityId}`;
