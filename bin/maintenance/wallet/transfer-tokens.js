...
async function printCurrentBalances(accountName) {
...
(async () => {
    ...
    const amount = 1;
    await TransactionSender.sendTokens(accountNameFrom, privateKey, accountNameTo, amount, '', 'UOSTEST');
    await TransactionSender.sendTokens(accountNameFrom, privateKey, accountNameTo, amount, '', 'GHTEST');
    await printCurrentBalances(accountNameFrom);
    await printCurrentBalances(accountNameTo);
})();
