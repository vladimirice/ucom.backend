...
class GithubSampleValues {
    static getAccessTokenForCode(code) {
        const set = {
        ...
        throw new errors_1.BadRequestError(`There is no token for code: ${code}`);
    }
    static getSampleExternalData(accessToken) {
        if (sampleData[accessToken]) {
            return sampleData[accessToken];
        }
        throw new errors_1.BadRequestError(`There is no sampleData for token: ${accessToken}`);
    }
    static getVladSampleExternalData() {
        return sampleData[VLAD_ACCESS_TOKEN];
    }
}
module.exports = GithubSampleValues;
